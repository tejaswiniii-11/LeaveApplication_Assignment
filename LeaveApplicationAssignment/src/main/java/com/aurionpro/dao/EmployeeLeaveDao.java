...
public class EmployeeLeaveDao {
    ...
    // Fetch leaves by status, e.g. "Pending"
    public List<EmployeeLeave> getLeavesByStatus(String status) {
        List<EmployeeLeave> leaveList = new ArrayList<>();
        String sql = "SELECT * FROM EmployeeLeave WHERE leave_status = ? ORDER BY leave_from DESC";
        try (Connection conn = Database.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    // Update status for a specific leave application by employee ID (as per your current model)
